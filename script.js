...
function LatticeCase(x, y) {
        switch(LatticeType) {
             case 1 :
                h = 30 * (4 * Math.PI) / (Math.sqrt(3) *  checkParameter(rowsTilt.value, 1, 4));
                a = Math.PI * checkParameter(parseFloat(TreshAngle.value), 30, 90) / 180;
                x = Math.sin(a) * h;
                koef = 0;
                koef2 = Math.sqrt(Math.pow(h , 2) - Math.pow(x , 2));
                return x;
             case 2 :
                x = 30 * (4 * Math.PI) / (Math.sqrt(3) *  checkParameter(rowsTilt.value, 1, 4));
                koef = 2;
                koef2 = 0;
                return x;
             case 3 :
                x = 30 * (4 * Math.PI) / (Math.sqrt(3) *  checkParameter(rowsTilt.value, 1, 4)); //change this for /2
                koef = 1;
                koef2 = 0; 
                return x; 
             default :
...
function rotate() {
    for (let i = 0; i <= 360; i++) {
        ...
        setTimeout(function () {
            ...
            drawAll(j);
            FinalResult.splice(i, 0, `Angle ${i}:${i < 10 ? '   ': i < 100 ? '  ' : ' '}` + angleSpectreString);
            if (i === 360) {
...
function drawAll(angle) {
    ...
    let columnTilt = parseFloat(columnsTilt.value); // circles in x/y
      ...
      columnTilt = 30 * (4 * Math.PI) / (Math.sqrt(3) *  checkParameter(columnTilt, 1, 4));
    ...
    for (let y = -canvas.height / 2 - yTilt + rowTilt * gjOleg2; y <= canvas.height / 2; y += rowTilt) {
        for (let x = (-canvas.width / 2 + (columnTilt / 2 * gjOleg) - columnTilt); x <= canvas.width / 2; x += columnTilt) {
            ...
            let fillPoint1;
            let fillPoint2;
            let pointTypeStatus = checkIfIntersected(redCircle.x, redCircle.y, newX, newY, redCircle.r, radius);
            if (pointTypeStatus && Math.abs(newX) <= canvas.width / 2 && Math.abs(newY) <= canvas.height / 2) {
                niceArray.push(fillPoint);
                recLineArray.push(intersectedDots(newX, newY, redCircle.x, redCircle.y, redCircle.r, radius));
...
singleResultButton.addEventListener('click', (ev) => {
    const initialData = `Electron energy: ${ewaldradius.value}, Ewald's Radius: ${0.512 * Math.sqrt(1000 * ewaldradius.value)}, Circle Radius: ${circleRadius.value}, Tilt: ${(4 * Math.PI) / (Math.sqrt(3) *  columnsTilt.value)} \n`;
    const singleCorrectData = angleSpectreString;
    writeTofile('singleResult.txt', initialData + "Angle " + inputAngle.value + ": " + singleCorrectData);
});

resultButton.addEventListener('click', (ev) => {
    ev.target.disabled = true;
    const initialData = `Electron energy: ${ewaldradius.value} * 10^3, Ewald's Radius: ${0.512 * Math.sqrt(1000 * ewaldradius.value)}, Circle Radius: ${circleRadius.value}, Tilt: ${(4 * Math.PI) / (Math.sqrt(3) *  columnsTilt.value)} \n`;
